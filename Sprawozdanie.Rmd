...
Natomiast $R^2$ oznacza ile danych zostało wyjaśnionych przez model. Im wartośc bliższa 1, tym lepszy model. 
...
<div style='text-align: justify'>
Dla pierwszych trzech badanych rozmiarów partii różnica nie wydaje się znacząca. Błędy walidacji są bardzo podobne i oscylują wokół 0.003. Natomiast zwiększenie rozmiaru partii do 128 próbek, wprowadza pogorszenie się wyników i zwiekszenie błędu walidacji. Możemy to również zauważyć, po wartościach RMSE i $R^2$. Widać również pewną zależność, że wraz ze wzrostem rozmiaru partii, nieznacznie maleje $R^2$, a RMSE rośnie. Nie są to co prawda duże różnice, ale są zauważalne. Dopasowanie modeli nie jest najgorsze, $R^2$ ~ 0.8 pokazuje, że duża część danych została wyjaśniona przez model. Natomiast błędy mogłyby być niższe.
</div>
...
<div style='text-align: justify'>
Ogólnie rzecz biorąc, bład walidacyjny w większości przypadków waha się wokół 0.001. Dla liczby neuronów = 200, jest on jednak niższy. We wszystkich sytuacjach występują duże wahania błędu walidacyjnego. 
