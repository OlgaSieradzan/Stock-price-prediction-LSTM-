...
W tabelce zestawiono RMSE i $\small R^2$ uzyskane w artykule (skrót art.) oraz nasz najlepszy wynik dla następujących wartości parametrów: 
...
Zastosowano również model regresji wektora nośnego (SVR), który stanowi adaptację SVM do problemów regresji, co czyni go bardziej odpowiednim do przewidywania wartości ciągłych, takich jak ceny akcji, w przeciwieństwie do pierwotnej funkcji SVM.
...
```{r, echo=FALSE}
methods <- c("LSTM", "LR", "SVR", "LSTM")
source <- c("Artykuł", "Artykuł", "Artykuł", "Nasz wynik")
RMSE <- c(1.57, 1.39, 7.25, 6.56)
R2 <- c(0.96, 1.00, 0.03, 0.97)
...
Mimo różnicy w liczbie rozważanych danych czasowych wartość $\small R^2$ dla LSTM w naszym badaniu oraz w przytoczonej pubikacji jest bardzo zbliżona. Widoczna jest tu znaczna różnica pomiędzy wartością dla SVR a pozostałymi modelami. Jeśli chodzi o wartości RMSE są one najbardziej podobne dla LSTM oraz regresji liniowej.  
</div>
